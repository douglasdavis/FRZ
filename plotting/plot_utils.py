...
def pad_margining(pad1,pad2):
    ...
    pad1.SetRightMargin(.025)
    pad2.SetRightMargin(.025)
...
leg_data.SetLineColorAlpha(ROOT.kBlack,1)
leg_data.SetMarkerStyle(8)
leg_zjets = ROOT.TH1D('leg_zjets','leg_zjets',100,1,0)
leg_zjets.SetLineWidth(2)
leg_zjets.SetLineColorAlpha(ROOT.kBlack,1)
leg_zjets.SetFillColorAlpha(0,1)
leg_diboson = ROOT.TH1D('leg_diboson','leg_diboson',100,1,0)
leg_diboson.SetLineWidth(0)
leg_diboson.SetFillColorAlpha(ROOT.kAzure+2,.7)
leg_diboson.SetLineColorAlpha(ROOT.kAzure+2,.7)
leg_ttbar = ROOT.TH1D('leg_ttbar','leg_ttbar',100,1,0)
leg_ttbar.SetLineWidth(0)
leg_ttbar.SetFillColorAlpha(ROOT.kGreen-2,.7)
leg_ttbar.SetLineColorAlpha(ROOT.kGreen-2,.7)
leg_ttbarV = ROOT.TH1D('leg_ttbarV','leg_ttbarV',100,1,0)
leg_ttbarV.SetLineWidth(0)
leg_ttbarV.SetFillColorAlpha(ROOT.kOrange+1,.7)
leg_ttbarV.SetLineColorAlpha(ROOT.kOrange+1,.7)

legend.AddEntry(leg_data,   'Data',      'p')
legend.AddEntry(leg_zjets,  'Z+jets',    'f')
legend.AddEntry(leg_diboson,'WZ/ZZ',     'f')
...
line_at_one.SetLineColorAlpha(1,1)
